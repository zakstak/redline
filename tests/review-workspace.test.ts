...
import type { CommentExportResponse, DiffResponse, ReviewDataResponse } from '../shared/review-contract.js';
...
beforeEach(async () => {
  ...
  await writeFile(join(repository, 'example.ts'), 'export const count = 1;\n', 'utf8');
  await runGit('add', 'example.ts');
  await runGit('commit', '-m', 'initial');
  await writeFile(join(repository, 'example.ts'), 'export const count = 2;\nexport const ready = true;\n', 'utf8');
});
...
describe('local review snapshots', () => {
  it('keeps an unchanged file approved and invalidates approval after its bytes change', async () => {
    ...
    await writeFile(join(repository, 'unrelated.txt'), 'land this separately\n', 'utf8');
    ...
    expect(afterUnrelatedHeadChange.files.find((file) => file.path === 'example.ts')?.reviewStatus).toBe('approved');

    await writeFile(join(repository, 'example.ts'), 'export const count = 3;\n', 'utf8');
  ...
  it('stores approvals and comments in Git metadata instead of the worktree', async () => {
    ...
    const status = await exec('git', ['status', '--porcelain'], { cwd: repository });
    ...
    const database = new DatabaseSync(join(repository, '.git', 'redline', 'review.sqlite'));
    try {
      expect(database.prepare('SELECT body FROM review_comments').get()).toMatchObject({
        body: 'Check this value.'
      });
      const columns = database.prepare('PRAGMA table_info(review_comments)').all() as unknown as Array<{ name: string }>;
  ...
  it('approves filenames that collide with Object prototype properties', async () => {
    ...
    try {
      ...
      expect(before.files.find((file) => file.path === 'toString')?.reviewStatus).toBe('unreviewed');
      expect(before.files.find((file) => file.path === '__proto__')?.reviewStatus).toBe('unreviewed');

      await workspace.approveSnapshot();
      const approved = await workspace.getWorkspace(true);
      expect(approved.files.find((file) => file.path === 'toString')?.reviewStatus).toBe('approved');
      expect(approved.files.find((file) => file.path === '__proto__')?.reviewStatus).toBe('approved');

      const stored = JSON.parse(
        await readFile(join(repository, '.git', 'redline', 'state.json'), 'utf8')
      ) as { approvals: Record<string, unknown> };
      expect(Object.keys(stored.approvals)).toEqual(expect.arrayContaining(['toString', '__proto__']));
    } finally {
      workspace.close();
    }
  });
  ...
  it('persists diff context settings in the workspace database', async () => {
    const workspace = new ReviewWorkspace(repository);
    await workspace.initialize();
    expect(await workspace.getSettings()).toEqual({ version: 1, diffContextLines: 3, keyboardLayout: 'normie' });
    expect(await workspace.updateSettings(8, 'vim')).toEqual({ version: 1, diffContextLines: 8, keyboardLayout: 'vim' });
    workspace.close();

    const reopened = new ReviewWorkspace(repository);
    await reopened.initialize();
    expect(await reopened.getSettings()).toEqual({ version: 1, diffContextLines: 8, keyboardLayout: 'vim' });
  ...
  it('emits a debounced local event when a worktree file changes', async () => {
    ...
    try {
      const changed = new Promise<{ paths: string[] }>((resolveEvent, reject) => {
        const timeout = setTimeout(() => reject(new Error('Timed out waiting for the workspace watcher.')), 3_000);
        unsubscribe = workspace.subscribeToChanges((event) => {
          clearTimeout(timeout);
          resolveEvent(event);
        });
      });
      await writeFile(join(repository, 'example.ts'), 'export const count = 77;\n', 'utf8');
  ...
  it('exposes validated settings through the local API', async () => {
    const baselineLines = Array.from({ length: 9 }, (_, index) => `export const value${index + 1} = ${index + 1};`);
    await writeFile(join(repository, 'example.ts'), `${baselineLines.join('\n')}\n`, 'utf8');
    await runGit('add', 'example.ts');
    ...
    try {
      ...
      expect(initial.json()).toEqual({ version: 1, diffContextLines: 3, keyboardLayout: 'normie' });
      ...
      expect(updated.json()).toEqual({ version: 1, diffContextLines: 12, keyboardLayout: 'vim' });
      ...
      const savedDefaultDiff = await app.inject({ method: 'GET', url: '/api/diff?path=example.ts' });
      expect(savedDefaultDiff.statusCode).toBe(200);
      expect(savedDefaultDiff.json<DiffResponse>().lines.filter((line) => line.type === 'context')).toHaveLength(0);

      const invalidContext = await app.inject({ method: 'GET', url: '/api/diff?path=example.ts&context=2.5' });
  ...
  it('exposes structured review data and Markdown comment export for local agents', async () => {
    ...
    try {
      ...
      expect(reviewResponse.json<ReviewDataResponse>()).toMatchObject({ version: 1 });
      expect(reviewResponse.json<ReviewDataResponse>().comments[0]).toMatchObject({
        path: 'example.ts',
        anchors: [{ side: 'new', startLine: 1, endLine: 2 }],
        body: 'Review this range.'
      });

      const diffResponse = await app.inject({ method: 'GET', url: '/api/diff?path=example.ts' });
      ...
      const agentExportResponse = await app.inject({ method: 'GET', url: '/api/comments/export' });
      expect(agentExportResponse.statusCode).toBe(200);
      expect(agentExportResponse.headers['content-type']).toContain('application/json');
      ...
      expect(agentMarkdownResponse.headers['content-type']).toContain('text/markdown');
  ...
  it('accepts side-aware comment anchors', async () => {
    const app = buildServer({ workspaceDir: repository });
    try {
      const currentDiff = await app.inject({ method: 'GET', url: '/api/diff?path=example.ts' });
      ...
      const refreshed = await app.inject({ method: 'GET', url: '/api/diff?path=example.ts' });
  ...
  it('hides generated and binary noise without losing its count', async () => {
    await writeFile(join(repository, 'artifact.png'), Buffer.from([0, 1, 2, 3]));
    await writeFile(join(repository, 'bundle.min.js'), 'const bundled=true;\n', 'utf8');
    const workspace = new ReviewWorkspace(repository);
    await workspace.initialize();
  ...
  it('rejects comments when the viewed fingerprint or anchors are stale', async () => {
    ...
    await writeFile(join(repository, 'example.ts'), 'export const count = 99;\n', 'utf8');

    await expect(workspace.addComment({
      path: 'example.ts',
  ...
  it('preserves no-newline metadata without shifting line anchors', async () => {
    await writeFile(join(repository, 'example.ts'), 'export const count = 2;', 'utf8');
    const workspace = new ReviewWorkspace(repository);
    await workspace.initialize();
  ...
  it('does not invent a source line for an empty file and marks a missing final newline', async () => {
    ...
    try {
      ...
      expect(noNewline.lines).toEqual(expect.arrayContaining([
        expect.objectContaining({ id: 'new-1', content: 'local text', noNewline: true })
      ]));
  ...
  it('reads only the displayed prefix of oversized untracked files', async () => {
    ...
    await writeFile(
      ...
      Buffer.concat([Buffer.alloc(limit, 97), Buffer.from('SECRET_AFTER_LIMIT')])
    );
    const workspace = new ReviewWorkspace(repository);
    await workspace.initialize();
  ...
  it('hides deleted binary files even when their extension is unknown', async () => {
    await writeFile(join(repository, 'binary.data'), Buffer.from([1, 0, 2, 0, 3]));
    ...
    const filtered = await workspace.getWorkspace();
    expect(filtered.files.map((file) => file.path)).not.toContain('binary.data');
    expect(filtered.hiddenNoiseCount).toBeGreaterThanOrEqual(1);
    const withNoise = await workspace.getWorkspace(true);
    expect(withNoise.files.find((file) => file.path === 'binary.data')).toMatchObject({
      binary: true,
      kind: 'deleted'
    });
    workspace.close();
  ...
  it('serializes concurrent approval writes without losing either file', async () => {
    ...
    expect(Object.keys(store.approvals).sort()).toEqual(['example.ts', 'second.ts']);
    workspace.close();
  });
  ...
  it('approves an explicit file set atomically and rejects the whole set when one fingerprint is stale', async () => {
    ...
    await writeFile(join(repository, 'example.ts'), 'export const count = 4;\n', 'utf8');
    await writeFile(join(repository, 'second.ts'), 'export const second = 2;\n', 'utf8');
    const workspace = new ReviewWorkspace(repository);
    await workspace.initialize();

    const viewed = (await workspace.getWorkspace(true)).files
      .filter((file) => file.path === 'example.ts' || file.path === 'second.ts');
    expect(viewed).toHaveLength(2);
    await writeFile(join(repository, 'example.ts'), 'export const count = 5;\n', 'utf8');

    await expect(workspace.approveFiles(viewed.map((file) => ({
      path: file.path,
      fingerprint: file.fingerprint
    })))).rejects.toThrow('Nothing was approved');
    const afterConflict = await workspace.getWorkspace(true);
    expect(afterConflict.files.find((file) => file.path === 'example.ts')?.reviewStatus).toBe('unreviewed');
    expect(afterConflict.files.find((file) => file.path === 'second.ts')?.reviewStatus).toBe('unreviewed');

    const current = afterConflict.files
      .filter((file) => file.path === 'example.ts' || file.path === 'second.ts');
    const result = await workspace.approveFiles(current.map((file) => ({
      path: file.path,
      fingerprint: file.fingerprint
    })));
    expect(result.approvals).toHaveLength(2);
    expect(new Set(result.approvals.map((approval) => approval.approvedAt))).toEqual(new Set([result.approvedAt]));
    const approved = await workspace.getWorkspace(true);
    expect(approved.files.filter((file) =>
      file.path === 'example.ts' || file.path === 'second.ts'
    ).every((file) => file.reviewStatus === 'approved')).toBe(true);
    workspace.close();
  });
  ...
  it('surfaces malformed approval state without overwriting it', async () => {
    ...
    await expect(workspace.initialize()).rejects.toThrow('Review state is malformed');
  ...
  it('rejects valid JSON with an invalid review state shape', async () => {
    ...
    await expect(workspace.initialize()).rejects.toThrow('approvals must be an object');
  ...
  it('invalidates approval when the executable mode changes', async () => {
    ...
    await workspace.initialize();
    const initial = await workspace.getWorkspace();
    const file = initial.files.find((candidate) => candidate.path === 'example.ts');
    expect(file).toBeDefined();
    await workspace.approveFile('example.ts', file?.fingerprint ?? '');

    await chmod(join(repository, 'example.ts'), 0o755);
    const changed = await workspace.getWorkspace();
    expect(changed.files.find((candidate) => candidate.path === 'example.ts')?.reviewStatus).toBe('changed');
    workspace.close();
  });

  it('invalidates approval when a submodule advances to another commit', async () => {
    const submoduleRepository = await mkdtemp(join(tmpdir(), 'redline-submodule-'));
    try {
      await exec('git', ['init'], { cwd: submoduleRepository });
      await exec('git', ['config', 'user.email', 'redline@example.test'], { cwd: submoduleRepository });
      await exec('git', ['config', 'user.name', 'Redline Test'], { cwd: submoduleRepository });
      ...
      const commitA = (await exec('git', ['rev-parse', 'HEAD'], { cwd: submoduleRepository })).stdout.trim();
      await writeFile(join(submoduleRepository, 'value.txt'), 'b\n', 'utf8');
      await exec('git', ['commit', '-am', 'b'], { cwd: submoduleRepository });
      const commitB = (await exec('git', ['rev-parse', 'HEAD'], { cwd: submoduleRepository })).stdout.trim();
      await writeFile(join(submoduleRepository, 'value.txt'), 'c\n', 'utf8');
      await exec('git', ['commit', '-am', 'c'], { cwd: submoduleRepository });
      const commitC = (await exec('git', ['rev-parse', 'HEAD'], { cwd: submoduleRepository })).stdout.trim();

      await exec('git', [
        '-c', 'protocol.file.allow=always', 'submodule', 'add', submoduleRepository, 'vendor/submodule'
      ], { cwd: repository });
      await exec('git', ['checkout', commitA], { cwd: join(repository, 'vendor', 'submodule') });
      await runGit('add', '.gitmodules', 'vendor/submodule');
      await runGit('commit', '-m', 'add submodule');
      await exec('git', ['checkout', commitB], { cwd: join(repository, 'vendor', 'submodule') });

      const workspace = new ReviewWorkspace(repository);
      await workspace.initialize();
      const before = await workspace.getWorkspace(true);
      const submodule = before.files.find((file) => file.path === 'vendor/submodule');
      expect(submodule).toBeDefined();
      await workspace.approveFile('vendor/submodule', submodule?.fingerprint ?? '');
      expect((await workspace.getWorkspace(true)).files.find(
        (file) => file.path === 'vendor/submodule'
      )?.reviewStatus).toBe('approved');

      await exec('git', ['checkout', commitC], { cwd: join(repository, 'vendor', 'submodule') });
  ...
  it('represents a mode-only diff without source-line anchors', async () => {
    ...
    const diff = await workspace.getDiff('example.ts');
    expect(diff.lines.map((line) => line.content)).toEqual(['old mode 100644', 'new mode 100755']);
    expect(diff.lines.every((line) => line.type === 'meta' && line.anchors.length === 0)).toBe(true);
    workspace.close();
  });

  it('preserves header-like source lines and their API anchors', async () => {
    await writeFile(join(repository, 'directives.txt'), '-- old directive\ntail\n', 'utf8');
    await runGit('add', 'directives.txt');
    await runGit('commit', '-m', 'add directive fixture');
    await writeFile(join(repository, 'directives.txt'), '++ new directive\ntail\n', 'utf8');
    ...
    try {
      ...
      expect(result.lines).toEqual(expect.arrayContaining([
        expect.objectContaining({ id: 'old-1', content: '-- old directive', oldLine: 1 }),
        expect.objectContaining({ id: 'new-1', content: '++ new directive', newLine: 1 }),
        expect.objectContaining({ id: 'both-2-2', content: 'tail', oldLine: 2, newLine: 2 })
      ]));
    } finally {
      workspace.close();
    }
  ...
  it('loads both sides of a pure rename instead of rendering a full-file addition', async () => {
    await writeFile(join(repository, 'before-name.ts'), 'export const renamed = true;\n', 'utf8');
    ...
    try {
      ...
      expect(state.files).toEqual(expect.arrayContaining([
        expect.objectContaining({ path: 'after-name.ts', originalPath: 'before-name.ts', kind: 'renamed' })
      ]));
  ...
  it('requires the viewed fingerprint when approving through the API', async () => {
    ...
    try {
      ...
      const viewed = await app.inject({ method: 'GET', url: '/api/diff?path=example.ts' });
      await writeFile(join(repository, 'example.ts'), 'export const count = 123;\n', 'utf8');
      const stale = await app.inject({
        method: 'POST',
        url: '/api/review/file',
        payload: { path: 'example.ts', fingerprint: viewed.json<DiffResponse>().fingerprint }
      });
      ...
      const staleBatch = await app.inject({
        method: 'POST',
        url: '/api/review/files',
        payload: { files: [{ path: 'example.ts', fingerprint: viewed.json<DiffResponse>().fingerprint }] }
      });
      expect(staleBatch.statusCode).toBe(409);
      expect(staleBatch.json<{ message: string }>().message).toContain('Nothing was approved');
    } finally {
      await app.close();
    }
  });
  ...
  it('models the net worktree without crashing on staged-add/delete or rename/delete states', async () => {
    await writeFile(join(repository, 'old-name.ts'), 'export const oldName = true;\n', 'utf8');
    await runGit('add', 'old-name.ts');
    await runGit('commit', '-m', 'add rename fixture');
    await writeFile(join(repository, 'example.ts'), 'export const count = 3;\n', 'utf8');

    await writeFile(join(repository, 'transient.ts'), 'export const transient = true;\n', 'utf8');
    ...
    try {
      await workspace.initialize();
      const state = await workspace.getWorkspace(true);
      expect(state.files.some((file) => file.path === 'transient.ts')).toBe(false);
      expect(state.files).toEqual(expect.arrayContaining([
        expect.objectContaining({ path: 'example.ts', kind: 'modified' }),
        expect.objectContaining({ path: 'old-name.ts', kind: 'deleted' })
      ]));
      expect(state.files.some((file) => file.path === 'new-name.ts')).toBe(false);
    } finally {
      workspace.close();
    }
  });
  ...
  it('retries a read that overlaps a workspace switch instead of mixing repositories', async () => {
    const secondRepository = await mkdtemp(join(tmpdir(), 'redline-read-switch-'));
    try {
      await exec('git', ['init'], { cwd: secondRepository });
      await exec('git', ['config', 'user.email', 'redline@example.test'], { cwd: secondRepository });
      await exec('git', ['config', 'user.name', 'Redline Test'], { cwd: secondRepository });
      await writeFile(join(secondRepository, 'second.ts'), 'export const second = 1;\n', 'utf8');
      await exec('git', ['add', 'second.ts'], { cwd: secondRepository });
      await exec('git', ['commit', '-m', 'initial'], { cwd: secondRepository });
      await writeFile(join(secondRepository, 'second.ts'), 'export const second = 2;\n', 'utf8');
  ...
  it('keeps the current workspace active when opening a target with corrupt review state', async () => {
    const corruptRepository = await mkdtemp(join(tmpdir(), 'redline-corrupt-open-'));
    try {
      await exec('git', ['init'], { cwd: corruptRepository });
      await exec('git', ['config', 'user.email', 'redline@example.test'], { cwd: corruptRepository });
      await exec('git', ['config', 'user.name', 'Redline Test'], { cwd: corruptRepository });
      await writeFile(join(corruptRepository, 'target.ts'), 'export const target = 1;\n', 'utf8');
      await exec('git', ['add', 'target.ts'], { cwd: corruptRepository });
      await exec('git', ['commit', '-m', 'initial'], { cwd: corruptRepository });
      await mkdir(join(corruptRepository, '.git', 'redline'), { recursive: true });
      await writeFile(join(corruptRepository, '.git', 'redline', 'state.json'), '{ malformed', 'utf8');

      const workspace = new ReviewWorkspace(repository);
      await workspace.initialize();
      await expect(workspace.openWorkspace(corruptRepository)).rejects.toThrow('malformed');
      const state = await workspace.getWorkspace(true);
  ...
  it('does not let a queued approval cross into a newly opened workspace', async () => {
    ...
    try {
      ...
      const workspace = new ReviewWorkspace(repository);
      await workspace.initialize();
      const firstFile = (await workspace.getWorkspace()).files.find((file) => file.path === 'example.ts');
      ...
      const writeStarted = new Promise<void>((resolveStarted) => { markWriteStarted = resolveStarted; });
      const writeGate = new Promise<void>((resolveWrite) => { releaseWrite = resolveWrite; });
      ...
      const approval = workspace.approveFile('example.ts', firstFile?.fingerprint ?? '');
      await writeStarted;
      const switching = workspace.openWorkspace(secondRepository);
      releaseWrite?.();
      await approval;
      ...
      const secondStatePath = join(secondRepository, '.git', 'redline', 'state.json');
