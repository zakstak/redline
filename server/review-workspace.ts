...
function validatePersistedStore(
  value: unknown,
  path: string,
): PersistedReviewStore {
  const invalid = (detail: string): never => {
    throw new Error(
      `Review state has an invalid shape at ${path}: ${detail}. The file was left untouched.`,
    );
  };
  ...
  if (!Array.isArray(store.snapshots))
    return invalid("snapshots must be an array");
  for (const snapshot of store.snapshots) {
    if (
      !isRecord(snapshot) ||
      typeof snapshot.id !== "string" ||
      typeof snapshot.approvedAt !== "string" ||
      typeof snapshot.head !== "string" ||
      !isRecord(snapshot.files) ||
...
function normalizeAnchors(anchors: ReviewAnchor[]): ReviewAnchor[] {
  ...
  for (const anchor of sorted) {
    ...
    if (
      ...
      anchor.startLine <= previous.endLine + 1
    ) {
...
function parseApiDiff(diff: string): DiffApiLine[] {
  ...
  rawLines.forEach((rawLine, index) => {
    ...
    if (rawLine === "\\ No newline at end of file") {
      ...
      if (previous && previous.type !== "hunk" && previous.type !== "meta")
...
export class ReviewWorkspace {
  ...
  async openWorkspace(requestedPath: string) {
    ...
    const operation = this.enqueueExclusive(async () => {
      ...
      await mkdir(dirname(targetDatabasePath), {
        recursive: true,
        mode: 0o700,
      });
  ...
  private startFileWatch(root: string, gitDir: string) {
    ...
    const addWatcher = (path: string, recursive: boolean, prefix = "") => {
      try {
        ...
        watcher.on("error", () => {
          watcher.close();
          this.fileWatchers = this.fileWatchers.filter(
  ...
  private enqueueExclusive<T>(operation: () => T | Promise<T>) {
    ...
    this.storeMutationQueue = queued.then(
      () => undefined,
      () => undefined,
    );
    return queued;
  ...
  private async readStable<T>(reader: () => Promise<T>): Promise<T> {
    ...
    throw new Error(
      "The active workspace changed while the request was being read. Try again.",
    );
  }
  ...
  private async currentHead() {
    const head = (
      await git(this.root, ["rev-parse", "--verify", "HEAD"], [0, 128])
    ).trim();
    return head || "unborn";
  }
  ...
  private async inspectFile(entry: StatusEntry) {
    ...
    const worktreeMode = fileStat.isSymbolicLink()
      ? "120000"
      : fileStat.isFile()
        ? Number(fileStat.mode & 0o111n) !== 0
          ? "100755"
          : "100644"
        : `special:${fileStat.mode}`;
    ...
    if (fileStat.isSymbolicLink()) {
      ...
      const inspected = await new Promise<{
        contentHash: string;
        binary: boolean;
      }>((resolveHash, reject) => {
  ...
  private async baselineBlobs(head: string, entries: StatusEntry[]) {
    ...
    const paths = [
      ...new Set(entries.map((entry) => entry.originalPath ?? entry.path)),
    ];
  ...
  private async deletedBinaryPaths(head: string, entries: StatusEntry[]) {
    ...
    const output = await git(this.root, [
      ...
      head,
      "--",
      ...deleted.map((entry) => `:(literal)${entry.path}`),
    ]);
    for (const record of output.split("\0")) {
  ...
  private async changedFiles(
    ...
    for (const cachedPath of this.inspectionCache.keys()) {
      if (!currentPaths.has(cachedPath))
        this.inspectionCache.delete(cachedPath);
    }
  ...
  private snapshotSummary(
    ...
    for (const file of files) {
      if (ownValue(snapshot.files, file.path) === file.fingerprint)
        unchangedCount += 1;
      else changedCount += 1;
    }
  ...
  private async getWorkspaceOnce(
    ...
    return {
      ...
      counts: {
        ...
        approved: visibleFiles.filter(
          (file) => file.reviewStatus === "approved",
        ).length,
        changed: visibleFiles.filter((file) => file.reviewStatus === "changed")
          .length,
        comments,
      },
      latestSnapshot: latestStoredSnapshot
        ? this.snapshotSummary(latestStoredSnapshot, visibleFiles)
        : null,
  ...
  private async getDiffOnce(path: string, context = 3): Promise<DiffResponse> {
    ...
    if (file.binary)
      throw new Error("Binary files do not have a readable text diff.");
    ...
    if (file.kind === "untracked" || workspace.head === "unborn") {
      ...
      if (fileStat.isSymbolicLink()) {
        ...
        throw new Error(
          "Only regular files and symbolic links have readable text diffs.",
        );
        ...
        diff = newFileDiff(
          path,
          await readUtf8Prefix(absolutePath, 5 * 1024 * 1024),
          mode,
          false,
        );
      } else {
  ...
  async approveFile(path: string, expectedFingerprint: string) {
    ...
    if (expectedFingerprint !== file.fingerprint) {
      throw new Error(
        "The file changed while it was open. Refresh the diff before approving it.",
      );
    }
  ...
  async approveFiles(
    requests: FileApprovalRequest[],
  ): Promise<FilesApprovalResponse> {
    await this.ensureInitialized();
    if (requests.length === 0)
      throw new Error("Choose at least one visible file to approve.");
    if (requests.length > 5_000)
      throw new Error("Approve no more than 5,000 visible files at once.");
    const uniquePaths = new Set(requests.map((request) => request.path));
    if (uniquePaths.size !== requests.length)
      throw new Error(
        "Each visible file can be approved only once per request.",
      );

    const generation = this.workspaceGeneration;
    const workspace = await this.getWorkspace(true);
    const filesByPath = new Map(
      workspace.files.map((file) => [file.path, file]),
    );
    ...
    for (const request of requests) {
      ...
      else if (!file || file.fingerprint !== request.fingerprint)
        stalePaths.push(request.path);
    }
    if (binaryPaths.length > 0) {
      throw new Error(
        `Binary files cannot be batch approved because they do not have a readable diff: ${binaryPaths.slice(0, 5).join(", ")}.`,
      );
    }
    if (stalePaths.length > 0) {
      ...
      throw new Error(
        `Nothing was approved. ${count} file${count === 1 ? "" : "s"} changed while the visible set was open: ${listed}${remainder}. Review ${count === 1 ? "it" : "them"} again.`,
      );
    }
  ...
  async approveSnapshot(): Promise<SnapshotResponse> {
    ...
    if (workspace.files.length === 0)
      throw new Error("There are no reviewable changes to approve.");
  ...
  async addComment(input: {
    ...
    if (body.length > 4000)
      throw new Error("Comments must be 4,000 characters or fewer.");

    const currentDiff = await this.getDiff(input.path, 20);
    if (input.expectedFingerprint !== currentDiff.fingerprint) {
      throw new Error(
        "The file changed while the comment was open. Refresh the diff before saving it.",
      );
    }
    ...
    if (!anchorsExist)
      throw new Error(
        "One or more comment lines are not present in the current diff.",
      );
    ...
    await this.enqueueExclusive(() => {
      if (generation !== this.workspaceGeneration) {
        throw new Error(
          "The active workspace changed before the comment could be saved.",
        );
      }
  ...
  async deleteComment(id: string) {
    ...
    await this.enqueueExclusive(() => {
      if (generation !== this.workspaceGeneration) {
        throw new Error(
          "The active workspace changed before the comment could be deleted.",
        );
      }
      if (!this.reviewDatabase().deleteComment(id))
  ...
  async getSettings(): Promise<ReviewSettings> {
    return this.readStable(() =>
      Promise.resolve(this.reviewDatabase().getSettings()),
    );
  }
  ...
  async updateSettings(
    ...
    await this.ensureInitialized();
    if (
      !Number.isSafeInteger(diffContextLines) ||
      diffContextLines < 0 ||
      diffContextLines > 20
    ) {
      throw new Error(
        "Unchanged context lines must be a whole number from 0 to 20.",
      );
    }
    ...
    return this.enqueueExclusive(() => {
      if (generation !== this.workspaceGeneration) {
        throw new Error(
          "The active workspace changed before settings could be saved.",
        );
      }
      return this.reviewDatabase().updateSettings(
        diffContextLines,
        keyboardLayout,
      );
    });
  ...
  private async getReviewMarkdownOnce() {
    ...
    for (const path of new Set(
      review.comments.map((comment) => comment.path),
    )) {
      try {
    ...
    for (const comment of review.comments) {
      ...
      if (fileDiff) {
        const context = fileDiff.lines.filter((line) =>
          line.anchors.some((lineAnchor) =>
            comment.anchors.some(
              (commentAnchor) =>
                lineAnchor.side === commentAnchor.side &&
                lineAnchor.startLine >=
                  Math.max(1, commentAnchor.startLine - 2) &&
        ...
        if (context.length > 0) {
          ...
          for (const line of context) {
            ...
            lines.push(
              `${marker} ${String(location).padStart(5, " ")} | ${line.content}`,
            );
          }
          lines.push("```");
          lines.push("");
