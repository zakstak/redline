...
import type { ReviewComment, ReviewSettings } from '../shared/review-contract.js';
...
export class ReviewDatabase {
  ...
  deleteComment(id: string) {
    const result = this.database.prepare('DELETE FROM review_comments WHERE id = ?').run(id);
    return Number(result.changes) > 0;
  }

  getSettings(): ReviewSettings {
    const contextRow = this.database.prepare(
      "SELECT value FROM review_settings WHERE key = 'diff_context_lines'"
    ).get() as { value?: string } | undefined;
    const keyboardRow = this.database.prepare(
      "SELECT value FROM review_settings WHERE key = 'keyboard_layout'"
    ).get() as { value?: string } | undefined;
    const parsed = Number(contextRow?.value ?? DEFAULT_CONTEXT_LINES);
    return {
      version: 1,
      diffContextLines: Number.isSafeInteger(parsed) && parsed >= 0 && parsed <= 20
        ? parsed
        : DEFAULT_CONTEXT_LINES,
      keyboardLayout: keyboardRow?.value === 'vim' ? 'vim' : DEFAULT_KEYBOARD_LAYOUT
    };
  }

  updateSettings(diffContextLines: number, keyboardLayout: ReviewSettings['keyboardLayout']): ReviewSettings {
