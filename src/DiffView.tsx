...
import { parseDiff, type DiffLine, type DiffRow, type WordSegment } from './diff-model.js';
...
export interface DiffViewHandle {
  ...
  navigateFocusedLine: (direction: 1 | -1, extend: boolean, count?: number) => void;
  navigateFocusedPage: (direction: 1 | -1, fraction: 0.5 | 1, extend: boolean) => void;
...
function styledText(content: string, style: CSSProperties | undefined, key: string): ReactNode {
  return style ? <span key={key} style={style}>{content}</span> : content;
}

function renderCode(segments: WordSegment[] | undefined, text: string, syntax: SyntaxToken[] | undefined) {
  if (!syntax) {
    if (!segments) return text;
    return segments.map((segment, index) => segment.changed ? (
      <mark className="word-change" key={`${segment.text}-${index}`}>{segment.text}</mark>
    ) : (
  ...
  if (!segments) {
    return syntax.map((token, index) => styledText(token.content, token.style, `syntax-${index}`));
  }
  ...
  syntax.forEach((token, tokenIndex) => {
    let tokenOffset = 0;
    while (tokenOffset < token.content.length && segmentIndex < segments.length) {
      ...
      output.push(segment.changed
        ? <mark className="word-change" key={key}>{node}</mark>
...
function commentCount(comments: ReviewComment[], line: DiffLine, side: 'old' | 'new') {
  const lineNumber = side === 'old' ? line.oldNumber : line.newNumber;
  return comments.filter((comment) => {
    return !comment.outdated && lineNumber !== null && comment.anchors.some((anchor) =>
      anchor.side === side && lineNumber >= anchor.startLine && lineNumber <= anchor.endLine
    );
  }).length;
}

function selectedLineFrom(line: DiffLine, preferredSide?: 'old' | 'new'): SelectedLine {
  ...
  return {
    ...
    side,
    label: `${side === 'old' ? 'Old' : 'New'} line ${resolvedNumber ?? ''}`.trim()
  };
}

function changeLabel(line: DiffLine) {
  if (line.type === 'add') return `Added line ${line.newNumber ?? ''}`.trim();
  if (line.type === 'remove') return `Removed line ${line.oldNumber ?? ''}`.trim();
...
function LineNumberButton({
  ...
  onSelectLine: (line: SelectedLine, event: MouseEvent<HTMLButtonElement>) => void;
}) {
  const number = side === 'old' ? line.oldNumber : line.newNumber;
...
function CodeContent({ line, syntax }: { line: DiffLine; syntax?: SyntaxToken[] }) {
  return (
    <code>
      ...
      {line.noNewline ? <span className="no-newline-label">no newline</span> : null}
    </code>
  );
}
...
function UnifiedLine({
  ...
  const searchHit = Boolean(search && line.text.toLowerCase().includes(search.toLowerCase()));

  if (line.type === 'meta') return <div className="diff-meta-line">{line.text}</div>;
  ...
  return (
    <div
      ...
      role="group"
    >
      <span aria-hidden="true" className="unified-old-number">{line.oldNumber ?? ''}</span>
...
function SideCell({
  ...
  if (!line) return <div aria-hidden="true" className="side-cell side-cell-empty" />;
  ...
  return (
    ...
    >
      {interactive ? (
        ...
        <span aria-hidden="true" className="side-line-number">{number ?? ''}</span>
      )}
      <CodeContent line={line} syntax={syntax} />
    </div>
  );
...
function HunkLine({ text, onExpandContext, canExpandContext }: { text: string; onExpandContext: () => void; canExpandContext: boolean }) {
  return (
    <div className="diff-hunk-line" role="separator">
      <span>{text}</span>
      {canExpandContext ? (
        <button onClick={onExpandContext} type="button">Show more context</button>
      ) : null}
    </div>
  );
}

function itemHeight(item: ActiveItem, rowHeight: number) {
  const type = item.type === 'unified' ? item.line.type : item.row.type;
  if (type === 'hunk') return HUNK_HEIGHT;
  if (type === 'meta') return META_HEIGHT;
  return rowHeight;
}
...
function itemLines(item: ActiveItem) {
  ...
  return [item.row.oldLine, item.row.newLine].filter((line): line is DiffLine => Boolean(line));
}

function findItemIndexForLine(items: ActiveItem[], lineId: string) {
  return items.findIndex((item) => itemLines(item).some((line) => line.id === lineId));
}

function useVirtualWindow(bodyRef: RefObject<HTMLDivElement | null>, heights: number[], scrolloffRows: number) {
  ...
  const [windowState, setWindowState] = useState({ start: 0, end: Math.min(80, heights.length) });
  ...
  const updateWindow = useCallback(() => {
    ...
    const first = Math.max(0, offsets.findIndex((offset, index) => index < heights.length && (offsets[index + 1] ?? offset) >= visibleTop));
    let last = first;
    while (last < heights.length && (offsets[last] ?? 0) <= visibleBottom) last += 1;
    setWindowState({
  ...
  const scrollToIndex = useCallback((index: number) => {
    ...
    const marginEndIndex = Math.min(heights.length, index + scrolloffRows + 1);
    const marginTop = (offsets[index] ?? 0) - (offsets[marginStartIndex] ?? 0);
    const marginBottom = (offsets[marginEndIndex] ?? bottom) - (offsets[index + 1] ?? bottom);
    if (top < viewportTop + marginTop) {
      scroller.scrollTo({ top: Math.max(0, top - marginTop), behavior: 'auto' });
    } else if (bottom > viewportBottom - marginBottom) {
      scroller.scrollTo({ top: Math.max(0, bottom + marginBottom - scroller.clientHeight), behavior: 'auto' });
    }
...
export const DiffView = forwardRef<DiffViewHandle, DiffViewProps>(function DiffView({
  ...
  const selectableLines = useMemo(() => parsed.lines.filter((line) =>
    line.type === 'add' || line.type === 'remove' || line.type === 'context'
  ), [parsed.lines]);
  const defaultFocusedLineId = selectableLines.find((line) => line.type === 'add')?.id ??
    selectableLines.find((line) => line.newNumber !== null)?.id ?? selectableLines[0]?.id ?? '';
  ...
  const canUseSplitView = parsed.lines.some((line) => line.type === 'add') &&
    parsed.lines.some((line) => line.type === 'remove');
  const effectiveView = view === 'adaptive' && canUseSplitView && availableWidth >= SPLIT_MIN_WIDTH
    ? 'split'
    : 'unified';
  const rowHeight = coarsePointer ? TOUCH_ROW_HEIGHT : DESKTOP_ROW_HEIGHT;
  const items = useMemo<ActiveItem[]>(() => effectiveView === 'split'
    ? parsed.rows.map((row) => ({ id: row.id, type: 'split', row }))
    : parsed.lines.map((line) => ({ id: line.id, type: 'unified', line })), [effectiveView, parsed.lines, parsed.rows]);
  const heights = useMemo(() => items.map((item) => itemHeight(item, rowHeight)), [items, rowHeight]);
  const {
    start: virtualStart,
    end: virtualEnd,
  ...
  const matchIndexes = useMemo(() => normalizedSearch
    ? items.flatMap((item, index) => itemText(item).toLowerCase().includes(normalizedSearch) ? [index] : [])
  ...
  const visibleCodeLines = useMemo(() => {
    const lines = visibleItems.flatMap(itemLines).filter((line) =>
      line.type === 'add' || line.type === 'remove' || line.type === 'context'
    );
  ...
  useEffect(() => {
    let cancelled = false;
    const missing = visibleCodeLines.filter((line) => !syntaxCacheRef.current.has(line.id));
    if (missing.length === 0) return;
    void highlightLines(missing.map((line) => line.text), diff.language)
      .then((highlighted) => {
        if (cancelled) return;
        missing.forEach((line, index) => syntaxCacheRef.current.set(line.id, highlighted[index] ?? []));
        setSyntaxVersion((current) => current + 1);
      })
      .catch(() => undefined);
    return () => {
  ...
  useEffect(() => {
    setFocusedLineId((current) => selectableLines.some((line) => line.id === current)
      ? current
  ...
  useEffect(() => {
    onSearchStatusChange({
      current: matchIndexes.length > 0 ? Math.min(activeMatch + 1, matchIndexes.length) : 0,
      total: matchIndexes.length
    });
  }, [activeMatch, matchIndexes.length, onSearchStatusChange]);

  const focusLine = useCallback((line: DiffLine | undefined) => {
    ...
    const button = rootRef.current?.querySelector<HTMLButtonElement>(`[data-line-id="${CSS.escape(line.id)}"]`);
    if (button) {
      button.focus();
  ...
  useEffect(() => {
    ...
    if (!lineId) return;
    const button = rootRef.current?.querySelector<HTMLButtonElement>(`[data-line-id="${CSS.escape(lineId)}"]`);
    if (!button) return;
    button.focus();
    pendingFocusLineIdRef.current = '';
  ...
  const navigateSearch = useCallback((direction: 1 | -1) => {
    ...
    setActiveMatch((current) => {
      const next = (current + direction + matchIndexes.length) % matchIndexes.length;
  ...
  const selectFocusedLine = useCallback(() => {
    const focusedLine = selectableLines.find((line) => line.id === focusedLineId) ?? selectableLines[0];
  ...
  const focusCurrentLine = useCallback(() => {
    const matchItem = normalizedSearch && matchIndexes.length > 0
      ? items[matchIndexes[Math.min(activeMatch, matchIndexes.length - 1)] ?? -1]
      : undefined;
    const matchLines = matchItem ? itemLines(matchItem).filter((line) =>
      line.type === 'add' || line.type === 'remove' || line.type === 'context'
    ) : [];
    const matchedLine = matchLines.find((line) => line.text.toLowerCase().includes(normalizedSearch)) ?? matchLines[0];
    const focusedLine = selectableLines.find((line) => line.id === focusedLineId) ?? selectableLines[0];
    return focusLine(matchedLine ?? focusedLine);
  }, [activeMatch, focusLine, focusedLineId, items, matchIndexes, normalizedSearch, selectableLines]);

  const goToLine = useCallback((lineNumber: number) => {
    const line = selectableLines.find((candidate) => candidate.newNumber === lineNumber) ??
      selectableLines.find((candidate) => candidate.oldNumber === lineNumber);
    return focusLine(line);
  }, [focusLine, selectableLines]);

  const selectRange = useCallback((anchorId: string, targetId: string) => {
    const anchorIndex = selectableLines.findIndex((candidate) => candidate.id === anchorId);
    const targetIndex = selectableLines.findIndex((candidate) => candidate.id === targetId);
    if (anchorIndex < 0 || targetIndex < 0) return false;
    const start = Math.min(anchorIndex, targetIndex);
    const end = Math.max(anchorIndex, targetIndex);
    onSelectionChange(selectableLines.slice(start, end + 1).map((line) => selectedLineFrom(line)));
    return true;
  }, [onSelectionChange, selectableLines]);

  const selectLine = useCallback((line: SelectedLine, event: MouseEvent<HTMLButtonElement>) => {
  ...
  const moveLineFocus = useCallback((lineId: string, direction: 1 | -1, extend: boolean) => {
    const currentIndex = selectableLines.findIndex((line) => line.id === lineId);
    if (currentIndex < 0) return;
    const nextIndex = Math.min(Math.max(currentIndex + direction, 0), selectableLines.length - 1);
  ...
  const navigateFocusedLine = useCallback((direction: 1 | -1, extend: boolean, count = 1) => {
    const currentIndex = Math.max(0, selectableLines.findIndex((line) => line.id === focusedLineId));
    ...
    if (!nextLine) return;
    if (extend) {
      const anchor = selectedLines[0]?.id ?? selectableLines[currentIndex]?.id;
      if (anchor) selectRange(anchor, nextLine.id);
    }
    focusLine(nextLine);
  }, [focusLine, focusedLineId, selectRange, selectableLines, selectedLines]);

  const navigateFocusedPage = useCallback((direction: 1 | -1, fraction: 0.5 | 1, extend: boolean) => {
    const scroller = rootRef.current?.closest('.diff-scroll');
    const visibleRows = Math.max(1, Math.floor((scroller?.clientHeight ?? rowHeight) / rowHeight));
    navigateFocusedLine(direction, extend, Math.max(1, Math.floor(visibleRows * fraction)));
  }, [navigateFocusedLine, rowHeight]);

  const focusBoundary = useCallback((boundary: 'start' | 'end', extend: boolean) => {
    const target = boundary === 'start' ? selectableLines[0] : selectableLines.at(-1);
  ...
  useImperativeHandle(ref, () => ({
    ...
    focusCurrentLine,
    goToLine
  }), [focusBoundary, focusCurrentLine, goToLine, navigateFocusedLine, navigateFocusedPage, navigateSearch, selectFocusedLine]);
  ...
  return (
    <div
      ...
      style={{
        ...
        '--diff-hunk-height': `${HUNK_HEIGHT}px`,
        '--diff-meta-height': `${META_HEIGHT}px`
      } as CSSProperties}
    >
      <div
        aria-keyshortcuts="c g"
        aria-label={effectiveView === 'split' ? 'Side by side diff' : 'Unified diff'}
      ...
      >
        ...
        <div className="virtual-diff-body" ref={bodyRef} style={{ height: totalHeight }}>
          <div className="virtual-diff-window" style={{ transform: `translateY(${virtualOffset}px)` }}>
            {visibleItems.map((item, visibleIndex) => {
              ...
              if (row.type === 'meta' && row.line) {
                return <div className="diff-meta-line" key={item.id}>{row.line.text}</div>;
              }
              return (
                <div className="split-row" key={item.id}>
                  <SideCell
                    ...
                    searchCurrent={searchCurrent}
                    side="old"
                    syntax={row.oldLine ? syntaxCacheRef.current.get(row.oldLine.id) : undefined}
                  />
                  <SideCell
                    interaction={interaction}
                    line={row.newLine}
                    ...
                    searchCurrent={searchCurrent}
                    side="new"
                    syntax={row.newLine ? syntaxCacheRef.current.get(row.newLine.id) : undefined}
                  />
                </div>
              );
            })}
