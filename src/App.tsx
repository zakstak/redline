...
async function api<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(payload.message || `Local request failed with status ${response.status}.`);
  }
...
function Icon({ children, size = 16 }: { children: ReactNode; size?: number }) {
  return (
    <svg aria-hidden="true" className="icon" height={size} viewBox="0 0 20 20" width={size}>
      {children}
    </svg>
  );
}
...
function CheckIcon() {
  return <Icon><path d="m4 10.5 3.5 3.5L16 5.5" /></Icon>;
}

function RefreshIcon() {
  return <Icon><path d="M16.1 7A6.5 6.5 0 1 0 16 13M16.1 7V3.5M16.1 7h-3.5" /></Icon>;
}

function SearchIcon() {
  return <Icon><circle cx="8.5" cy="8.5" r="5" /><path d="m12.3 12.3 4 4" /></Icon>;
}

function CommentIcon() {
  return <Icon><path d="M4 4.5h12v8H9l-4.5 3v-3H4z" /></Icon>;
}

function SnapshotIcon() {
  return <Icon><path d="M5 5.5h10v10H5z" /><path d="M7.5 3h10v10M8 10l1.5 1.5L12.5 8" /></Icon>;
}

function PreviousIcon() {
  return <Icon size={14}><path d="m5 12 5-5 5 5" /></Icon>;
}

function NextIcon() {
  return <Icon size={14}><path d="m5 8 5 5 5-5" /></Icon>;
}

function LockIcon() {
  return <Icon><rect x="4.5" y="8" width="11" height="8" rx="1.5" /><path d="M7 8V6a3 3 0 0 1 6 0v2" /></Icon>;
}

function ChevronIcon() {
  return <Icon><path d="m7 4 6 6-6 6" /></Icon>;
}

function LeftPanelIcon() {
  return <Icon><rect x="2.5" y="3" width="15" height="14" rx="1.5" /><path d="M7 3v14" /></Icon>;
}

function RightPanelIcon() {
  return <Icon><rect x="2.5" y="3" width="15" height="14" rx="1.5" /><path d="M13 3v14" /></Icon>;
}

function SettingsIcon() {
  return <Icon><circle cx="10" cy="10" r="2.5" /><path d="M10 2.8v2M10 15.2v2M2.8 10h2M15.2 10h2M4.9 4.9l1.4 1.4M13.7 13.7l1.4 1.4M15.1 4.9l-1.4 1.4M6.3 13.7l-1.4 1.4" /></Icon>;
}

function BackIcon() {
  return <Icon><path d="m12.5 4-6 6 6 6" /></Icon>;
}

function DatabaseIcon() {
  return <Icon><ellipse cx="10" cy="5" rx="6" ry="2.5" /><path d="M4 5v5c0 1.4 2.7 2.5 6 2.5s6-1.1 6-2.5V5M4 10v5c0 1.4 2.7 2.5 6 2.5s6-1.1 6-2.5v-5" /></Icon>;
}
...
function selectionLabel(lines: SelectedLine[]) {
  ...
  if (sides.size > 1) return `${lines.length} lines selected across before and after`;
  const numbers = lines
...
function commentLineLabel(comment: ReviewComment) {
  return comment.anchors.map((anchor) => {
  ...
  return (
    <div className="loading-shell" aria-label="Loading local workspace">
      <aside className="loading-files">
        ...
        {Array.from({ length: 7 }, (_, index) => <span className="skeleton skeleton-file" key={index} />)}
      </aside>
      <section className="loading-diff">
        <span className="skeleton skeleton-heading" />
        {Array.from({ length: 13 }, (_, index) => (
          <span className="skeleton skeleton-code" key={index} style={{ width: `${48 + ((index * 17) % 43)}%` }} />
        ))}
      </section>
      <aside className="loading-ledger"><span className="skeleton skeleton-title" /></aside>
    </div>
  );
}
...
function SettingsPage({
  ...
  const [keyboardLayout, setKeyboardLayout] = useState<ReviewSettings['keyboardLayout']>(settings.keyboardLayout);
  const [saving, setSaving] = useState(false);
  const [saveState, setSaveState] = useState<'idle' | 'saved' | 'error'>('idle');
  const parsedDraft = Number(draft);
  const validDraft = /^\d+$/.test(draft) && Number.isSafeInteger(parsedDraft) && parsedDraft >= 0 && parsedDraft <= 20;
  const dirty = validDraft && (
    parsedDraft !== settings.diffContextLines || keyboardLayout !== settings.keyboardLayout
  );
  ...
  return (
    <main className="app-shell settings-shell" data-shell="settings">
      <header className="settings-header">
        <button autoFocus className="settings-back-button" onClick={onBack} type="button">
          <BackIcon /> Review
        </button>
        <div>
          <span>Settings</span>
        ...
        <span className="settings-local-label"><LockIcon /> Local only</span>
      </header>
      ...
      <form className="settings-page" onSubmit={(event) => void save(event)}>
        <div className="settings-intro">
          ...
          <p>These defaults apply to every file in this workspace. You can still expand individual hunks while reviewing.</p>
        </div>

        <section className="settings-section" aria-labelledby="context-lines-heading">
          ...
          <div className="context-setting-control">
            <div aria-label="Common context line values" className="context-presets" role="group">
            ...
            <p id="context-lines-help">0 shows only changed lines. Maximum 20.</p>
          </div>

          <div aria-label="Diff context preview" className="context-preview">
            <div><span>{validDraft ? parsedDraft : '–'}</span> unchanged lines</div>
            <div><span>+</span> changed line</div>
            <div><span>{validDraft ? parsedDraft : '–'}</span> unchanged lines</div>
          </div>
        </section>

        <section className="settings-section keyboard-layout-section" aria-labelledby="keyboard-layout-heading">
          <div className="settings-section-copy">
            <h2 id="keyboard-layout-heading">Keyboard layout</h2>
            <p>Choose familiar browser controls or modal diff navigation. Pointer controls work in both layouts.</p>
          </div>

          <div aria-label="Keyboard layout" className="keyboard-layout-options" role="radiogroup">
            <button
              ...
              type="button"
            >
              <span><strong>Normie</strong><small>Tab, arrows, click</small></span>
              <kbd>Default</kbd>
            </button>
            <button
              ...
              type="button"
            >
              <span><strong>Vim</strong><small>J/K, V, C, Esc, A</small></span>
              <kbd>Modal</kbd>
            </button>
          </div>
        </section>
        ...
        <section className="settings-storage" aria-labelledby="storage-heading">
          ...
          <div>
            ...
            <p>Comments and these preferences stay in <code>.git/redline/review.sqlite</code>. Repository contents never leave this machine.</p>
          </div>
        </section>

        <footer className="settings-actions">
          <p aria-live="polite">
            {!validDraft ? 'Enter a whole number from 0 to 20.' : saveState === 'saved' ? 'Saved for this workspace.' : saveState === 'error' ? 'Settings could not be saved.' : dirty ? 'Unsaved change.' : 'Settings are current.'}
          </p>
...
export default function App() {
  ...
  const [diffSearchStatus, setDiffSearchStatus] = useState<DiffSearchStatus>({ current: 0, total: 0 });
  ...
  const [filePanelOpen, setFilePanelOpen] = useState(() => !window.matchMedia('(max-width: 55.99rem)').matches);
  ...
  const [pendingReviewFocus, setPendingReviewFocus] = useState<'panel' | 'composer' | null>(null);
  const [pendingFileSearch, setPendingFileSearch] = useState(false);
  const [settings, setSettings] = useState<ReviewSettings>({ version: 1, diffContextLines: 3, keyboardLayout: 'normie' });
  ...
  const [storedSelectedLines, setStoredSelectedLines] = useState<SelectedLine[]>([]);
  const [selectionScope, setSelectionScope] = useState<{ path: string; fingerprint: string } | null>(null);
  ...
  const [reviewHintVisible, setReviewHintVisible] = useState(() => !reviewHintWasDismissed());
  ...
  const previousActiveFileRef = useRef<{ path: string; fingerprint: string } | null>(null);
  ...
  const loadWorkspace = useCallback(async (silent = false) => {
    ...
    try {
      const nextWorkspace = await api<WorkspaceResponse>(`/api/workspace?includeNoise=${includeNoise}`);
      if (requestId !== workspaceRequestRef.current || workspaceOpenRef.current) return null;
      setWorkspace(nextWorkspace);
      setPathInput(nextWorkspace.root);
      setWorkspaceError('');
      setActivePath((current) => {
        if (nextWorkspace.files.some((file) => file.path === current)) return current;
        return nextWorkspace.files.find((file) => file.reviewStatus !== 'approved')?.path ?? nextWorkspace.files[0]?.path ?? '';
      });
      return nextWorkspace;
    } catch (error) {
      if (requestId !== workspaceRequestRef.current || workspaceOpenRef.current) return null;
      setWorkspaceError(error instanceof Error ? error.message : 'The local workspace could not be loaded.');
      return null;
    } finally {
      if (requestId === workspaceRequestRef.current && !workspaceOpenRef.current) {
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      ...
      if (requestId !== settingsRequestRef.current) return null;
      setActionError(error instanceof Error ? error.message : 'Review settings could not be loaded.');
      return null;
    }
  }, []);
  ...
  const displayedDiff = diff?.path === activePath && diffWorkspaceRoot === workspace?.root ? diff : null;
  const selectionIsCurrent = Boolean(
    displayedDiff && selectionScope?.path === displayedDiff.path &&
    selectionScope.fingerprint === displayedDiff.fingerprint
  );
  const selectedLines = selectionIsCurrent ? storedSelectedLines : [];
  draftStateRef.current.hasDraft = storedSelectedLines.length > 0 || Boolean(commentBody.trim());
  const setSelectedLines = useCallback((lines: SelectedLine[]) => {
    setStoredSelectedLines(lines);
    setSelectionScope(lines.length > 0 && displayedDiff
  ...
  useEffect(() => {
    ...
    const current = activeFingerprint ? { path: activePath, fingerprint: activeFingerprint } : null;
    const fileChangedInPlace = Boolean(
      previous && current && previous.path === current.path &&
      previous.fingerprint !== current.fingerprint
    );
    if (fileChangedInPlace && draftStateRef.current.hasDraft) {
      setDraftInvalidationNotice('This file changed. The unsaved comment anchor was cleared; existing notes are kept as stale history.');
  ...
  const loadDiff = useCallback(async () => {
    ...
    try {
      ...
      setDiff(null);
      setDiffWorkspaceRoot('');
      setActionError(error instanceof Error ? error.message : 'The diff could not be loaded.');
    } finally {
  ...
  const visibleApprovalFiles = useMemo(
    () => visibleFiles.filter((file) => file.reviewStatus !== 'approved' && !file.binary),
    [visibleFiles]
  );
  const visibleBinaryCount = useMemo(
    () => visibleFiles.filter((file) => file.reviewStatus !== 'approved' && file.binary).length,
    [visibleFiles]
  );

  const selectRelativeFile = useCallback((direction: 1 | -1) => {
    if (visibleFiles.length === 0) return;
    const currentIndex = visibleFiles.findIndex((file) => file.path === activePath);
  ...
  const closeFilePanel = useCallback((restoreFocus = true) => {
    ...
    if (restoreFocus) {
      window.requestAnimationFrame(() => window.requestAnimationFrame(() => {
        (filePanelToggleRef.current ?? filePanelReturnFocusRef.current)?.focus();
      }));
    }
  }, []);

  const openFilePanel = useCallback((focusSearch = true) => {
    if (!filePanelOpen) filePanelReturnFocusRef.current = document.activeElement as HTMLElement | null;
  ...
  const closeReviewPanel = useCallback((restoreFocus = true) => {
    ...
    if (restoreFocus) window.requestAnimationFrame(() => window.requestAnimationFrame(() => {
      reviewToggleRef.current?.focus();
    }));
  }, []);

  const openReviewPanel = useCallback((focus: 'panel' | 'composer' = 'panel') => {
  ...
  const openLineJump = useCallback(() => {
    ...
    setLineJumpValue('');
    setLineJumpError('');
    window.requestAnimationFrame(() => window.requestAnimationFrame(() => lineJumpRef.current?.focus()));
  ...
  useEffect(() => {
    ...
    window.requestAnimationFrame(() => {
      if (pendingReviewFocus === 'composer') commentTextareaRef.current?.focus();
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Escape') {
        ...
        else if (!target.matches('input, textarea, select, [contenteditable="true"]')) {
      ...
      if (settings.keyboardLayout === 'vim' && vimDiffMode) {
        ...
        if (event.ctrlKey && (key === 'd' || key === 'u')) {
          event.preventDefault();
          diffViewRef.current?.navigateFocusedPage(key === 'd' ? 1 : -1, 0.5, vimVisualMode);
          return;
        }
        if (event.key === 'PageDown' || event.key === 'PageUp') {
          event.preventDefault();
          diffViewRef.current?.navigateFocusedPage(event.key === 'PageDown' ? 1 : -1, 1, vimVisualMode);
          return;
        }
        ...
        if (event.key === 'g' && !event.shiftKey && !event.metaKey && !event.ctrlKey && !event.altKey) {
          event.preventDefault();
          const now = Date.now();
        ...
        if ((key === 'j' || key === 'k') && !event.metaKey && !event.ctrlKey && !event.altKey) {
          event.preventDefault();
          diffViewRef.current?.navigateFocusedLine(key === 'j' ? 1 : -1, vimVisualMode);
          return;
        }
        ...
        if (key === 'c' && !event.metaKey && !event.ctrlKey && !event.altKey && displayedDiff) {
      ...
      if (
        settings.keyboardLayout === 'vim' && !vimDiffMode && event.key === 'Enter' && displayedDiff &&
        (target === document.body || target.matches('.line-number-button') || Boolean(target.closest('.diff-view')))
      ) {
        event.preventDefault();
        setVimDiffMode(true);
        setVimVisualMode(false);
      ...
      if (event.key === '/') {
        ...
        if (reviewPanelOpen) closeReviewPanel(false);
        setLineJumpOpen(false);
        window.requestAnimationFrame(() => window.requestAnimationFrame(() => diffSearchRef.current?.focus()));
      }
      if (
  ...
  }, [activePage, closeFilePanel, closeReviewPanel, diffSearch, displayedDiff, filePanelOpen, filePanelOverlay, fileSearch, openFilePanel, openLineJump, openReviewPanel, reviewPanelOpen, selectRelativeFile, selectedLines.length, setSelectedLines, settings.keyboardLayout, vimDiffMode, vimVisualMode]);
  ...
  useEffect(() => {
    ...
    const handleWheel = (event: WheelEvent) => {
      ...
      diffViewRef.current?.navigateFocusedLine(rowCount > 0 ? 1 : -1, vimVisualMode, Math.min(6, Math.abs(rowCount)));
    };
  ...
  const openWorkspace = async (event: FormEvent) => {
    ...
    try {
      const nextWorkspace = await api<WorkspaceResponse>('/api/workspace/open', {
        method: 'POST',
      ...
      setActivePath(nextWorkspace.files.find((file) => file.reviewStatus !== 'approved')?.path ?? nextWorkspace.files[0]?.path ?? '');
      setSelectedLines([]);
      await loadSettings();
    } catch (error) {
      setWorkspaceError(error instanceof Error ? error.message : 'That local workspace could not be opened.');
      void loadDiff();
    } finally {
  ...
  const approveFile = async () => {
    ...
    try {
      await api('/api/review/file', {
        method: 'POST',
        body: JSON.stringify({ path: displayedDiff.path, fingerprint: displayedDiff.fingerprint })
      });
      setDiff((current) => current?.path === displayedDiff.path
        ? { ...current, reviewStatus: 'approved', approvedAt: new Date().toISOString() }
        : current);
      const refreshed = await loadWorkspace(true);
      const nextFile = refreshed?.files.find((file) => file.reviewStatus !== 'approved' && file.path !== displayedDiff.path);
      if (nextFile && filter === 'needs-review') setActivePath(nextFile.path);
      setActionError('');
    } catch (error) {
      setActionError(error instanceof Error ? error.message : 'The file could not be approved.');
  ...
  const approveSnapshot = async () => {
    ...
    try {
      const result = await api<SnapshotResponse>('/api/review/snapshot', { method: 'POST', body: '{}' });
      setWorkspace(result.workspace);
      setDiff((current) => current ? { ...current, reviewStatus: 'approved', approvedAt: result.snapshot.approvedAt } : current);
      setActionError('');
    } catch (error) {
      setActionError(error instanceof Error ? error.message : 'The snapshot could not be approved.');
    } finally {
      setApproving('');
    }
  };
  ...
  const approveVisibleFiles = async () => {
    ...
    try {
      const result = await api<FilesApprovalResponse>('/api/review/files', {
        method: 'POST',
        body: JSON.stringify({
          files: files.map((file) => ({ path: file.path, fingerprint: file.fingerprint }))
        })
      });
      const approvedPaths = new Set(result.approvals.map((approval) => approval.path));
      setDiff((current) => current && approvedPaths.has(current.path)
        ? { ...current, reviewStatus: 'approved', approvedAt: result.approvedAt }
        : current);
      await loadWorkspace(true);
      const count = result.approvals.length;
      setVisibleApprovalMessage(`${count} visible file${count === 1 ? '' : 's'} approved.`);
      window.requestAnimationFrame(() => {
      ...
      setVisibleApprovalError(error instanceof Error ? error.message : 'The visible files could not be approved.');
      await loadWorkspace(true);
    } finally {
  ...
  const addComment = async (event: FormEvent) => {
    event.preventDefault();
    if (!displayedDiff || displayedDiff.path !== activePath || selectedLines.length === 0) return;
    setSavingComment(true);
    try {
      const anchors = selectedLines.reduce<Array<{ side: 'old' | 'new'; startLine: number; endLine: number }>>(
        (ranges, line) => {
          if (line.number === null) return ranges;
          const previous = ranges.at(-1);
          if (previous && previous.side === line.side && line.number <= previous.endLine + 1) {
            previous.endLine = Math.max(previous.endLine, line.number);
          } else {
            ranges.push({ side: line.side, startLine: line.number, endLine: line.number });
          }
          return ranges;
        },
        []
      ...
      setActionError(error instanceof Error ? error.message : 'The comment could not be saved.');
    } finally {
      setSavingComment(false);
    }
  };

  const commitCommentDeletion = useCallback(async (pending: PendingCommentDeletion) => {
    if (pendingCommentDeletionRef.current?.comment.id !== pending.comment.id) return;
    if (commentDeletionTimerRef.current !== null) window.clearTimeout(commentDeletionTimerRef.current);
    commentDeletionTimerRef.current = null;
    pendingCommentDeletionRef.current = null;
    setPendingCommentDeletion(null);
    try {
      await api(`/api/comments/${encodeURIComponent(pending.comment.id)}`, { method: 'DELETE' });
      if (activeWorkspaceRootRef.current === pending.workspaceRoot) await loadWorkspace(true);
    } catch (error) {
      if (activeWorkspaceRootRef.current === pending.workspaceRoot) {
        setDiff((current) => current?.path === pending.comment.path &&
          !current.comments.some((comment) => comment.id === pending.comment.id)
          ? {
              ...current,
              comments: [...current.comments, pending.comment]
  ...
  const scheduleCommentDeletion = useCallback((comment: ReviewComment) => {
    const previous = pendingCommentDeletionRef.current;
    if (previous) void commitCommentDeletion(previous);
    const pending = { comment, workspaceRoot: activeWorkspaceRootRef.current };
    pendingCommentDeletionRef.current = pending;
    setPendingCommentDeletion(pending);
    setDiff((current) => current?.path === comment.path ? {
      ...current,
  ...
  useEffect(() => () => {
    if (commentDeletionTimerRef.current !== null) window.clearTimeout(commentDeletionTimerRef.current);
    if (visibleApprovalMessageTimerRef.current !== null) window.clearTimeout(visibleApprovalMessageTimerRef.current);
  }, []);
      ...
      <main className="app-shell" data-shell="review-workspace">
        <section className="workspace-error-state">
          <div>
            ...
            <p>{workspaceError || 'The current server directory is not a Git workspace.'}</p>
            <WorkspaceForm
  ...
  return (
    <main className="app-shell" data-shell="review-workspace">
      <a className="skip-link" href="#diff-content">Skip to diff</a>

      <div className="review-shell" data-files-open={filePanelOpen} data-review-open={reviewPanelOpen}>
        <aside
        ...
        >
          <div className="workspace-identity">
            ...
            {workspacePathOpen ? (
              <form className="workspace-path-form" onSubmit={(event) => void openWorkspace(event)}>
                ...
                <div>
                  <input
                    ...
                    value={pathInput}
                  />
                  <button disabled={openingWorkspace || pathInput === workspace.root} type="submit">Open</button>
                </div>
              </form>
            ) : null}
          </div>
          ...
          <div className="review-counts" aria-label="Review counts">
            <button data-active={filter === 'needs-review'} onClick={() => setFilter('needs-review')} type="button">
              <strong>{workspace.counts.needsReview}</strong>
              <span>Needs review</span>
            </button>
            <button data-active={filter === 'approved'} onClick={() => setFilter('approved')} type="button">
              <strong>{workspace.counts.approved}</strong>
              <span>Approved</span>
            </button>
            <button data-active={filter === 'all'} onClick={() => setFilter('all')} type="button">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (filePanelOverlay) {
                      ...
                      window.requestAnimationFrame(() => document.getElementById('diff-content')?.focus());
                    }
                  }}
                  type="button"
                >
                  <span className="file-status-mark" aria-hidden="true">
                    {file.reviewStatus === 'approved' ? <CheckIcon /> : file.reviewStatus === 'changed' ? '↻' : '•'}
                  </span>
                  ...
                  <span className="file-meta">
                    ...
                    {file.commentCount > 0 ? <span className="file-comment-count"><CommentIcon />{file.commentCount}</span> : null}
                  </span>
                  <span className="visually-hidden">{statusLabel(file)}, {changeLabel(file)}</span>
                </button>
              </div>
            )) : (
              <div className="file-list-empty">
                {filter === 'needs-review' && workspace.counts.approved > 0 ? (
                  <>
                    ...
                    <span>Approved files return here only when they change.</span>
                  </>
                ) : (
                  <>
                    <SearchIcon />
          ...
          {visibleApprovalFiles.length >= 2 || visibleApprovalMessage || visibleApprovalError ? (
            <section className="visible-approval" aria-label="Visible file approval">
              {visibleApprovalFiles.length >= 2 ? (
                ...
                >
                  <CheckIcon />
                  <span>
                    <strong>{approvingVisibleCount > 0
                      ? `Approving ${approvingVisibleCount} files`
                      : `Approve ${visibleApprovalFiles.length} visible files`}</strong>
                    <small>Current filtered queue. Does not stage or commit.</small>
                  </span>
                </button>
              ) : null}
              {visibleBinaryCount > 0 && visibleApprovalFiles.length >= 2 ? (
                <p>{visibleBinaryCount} binary file{visibleBinaryCount === 1 ? '' : 's'} excluded.</p>
              ) : null}
              <p aria-live="polite" className="visible-approval-result" data-error={Boolean(visibleApprovalError)}>
          ...
          <div className="rail-footer">
            <label className="noise-toggle">
              <input checked={includeNoise} onChange={(event) => setIncludeNoise(event.target.checked)} type="checkbox" />
              <span className="toggle-track"><span /></span>
              <span>
                Show review noise
                <small>{workspace.hiddenNoiseCount} generated or binary hidden</small>
              </span>
            </label>
            <button
            ...
            >
              <SettingsIcon />
              <span>Settings</span>
              <small>{settings.keyboardLayout === 'vim' ? 'Vim' : 'Normie'} · {settings.diffContextLines} lines</small>
            </button>
            <div className="workspace-watch-state" data-state={watchState}>
              <span aria-hidden="true" />
              {refreshing ? 'Refreshing changes' : watchState === 'live' ? 'Watching files' : watchState === 'polling' ? 'Polling for changes' : 'Connecting watcher'}
            </div>
            <details className="shortcut-guide">
              ...
              <div aria-label="Keyboard shortcuts">
                {settings.keyboardLayout === 'vim' ? (
                  <>
                    <span><kbd>Enter</kbd> Navigate the diff</span>
                    <span><kbd>J</kbd><kbd>K</kbd> Move files or diff lines</span>
                    <span><kbd>V</kbd> Select lines in diff mode</span>
                    <span><kbd>Ctrl+D</kbd><kbd>Ctrl+U</kbd> Half-page</span>
                    <span><kbd>gg</kbd><kbd>G</kbd> First or last line</span>
                    <span><kbd>C</kbd> Comment on selected lines</span>
                    <span><kbd>A</kbd> Approve in diff mode</span>
                  </>
                ) : (
                  <>
                    <span><kbd>J</kbd><kbd>K</kbd> Next or previous file</span>
                    <span><kbd>C</kbd> Comment on selected lines</span>
                  </>
                )}
                <span><kbd>/</kbd> Search this diff</span>
                <span><kbd>G</kbd> Go to a visible line</span>
                <span><kbd>[</kbd><kbd>]</kbd> Toggle side panels</span>
              </div>
            </details>
          </div>
        </aside>
        ...
        <section
          ...
          id="diff-content"
          inert={(filePanelOverlay && filePanelOpen) || (reviewPanelOverlay && reviewPanelOpen) ? true : undefined}
          tabIndex={-1}
        >
            ...
            <>
              <header className="diff-toolbar">
                <div className="active-file-heading">
                  ...
                  <div className="file-breadcrumb">
                    {activeFile.directory ? <span>{activeFile.directory} /</span> : null}
                    <strong>{activeFile.name}</strong>
                  </div>
                  <div className="active-file-meta">
                    <span className="change-kind">{changeLabel(activeFile)}</span>
                    {displayedDiff ? <><span className="added-stat">+{displayedDiff.stats.additions}</span><span className="deleted-stat">−{displayedDiff.stats.deletions}</span></> : null}
                    {activeFile.reviewStatus === 'changed' ? <span className="changed-warning">Changed since approval</span> : null}
                  </div>
                </div>
                ...
                <div className="diff-tools">
                  <div className="diff-search-control">
                    {lineJumpOpen ? (
                      <label className="diff-search line-jump">
                        <span aria-hidden="true" className="line-jump-mark">#</span>
                        <span className="visually-hidden">Go to visible diff line</span>
                        <input
                          aria-describedby={lineJumpError ? 'line-jump-error' : undefined}
                          ...
                          value={lineJumpValue}
                        />
                        {lineJumpError ? <span className="line-jump-status" id="line-jump-error" role="alert">Not visible</span> : <kbd>Enter</kbd>}
                      </label>
                    ) : (
                      <label className="diff-search">
                        <span className="visually-hidden">Search this diff</span>
                        <SearchIcon />
                        <input
                          ...
                          onKeyDown={(event) => {
                            if (event.key === 'Enter') {
                              event.preventDefault();
                              diffViewRef.current?.navigateSearch(event.shiftKey ? -1 : 1);
                            }
                            if (event.key === 'Escape') {
                              event.preventDefault();
                              event.stopPropagation();
                        ...
                        />
                        {diffSearch ? (
                          <span aria-live="polite" className="diff-search-count">
                            {diffSearchStatus.total ? `${diffSearchStatus.current}/${diffSearchStatus.total}` : '0/0'}
                          </span>
                        ) : null}
                      </label>
                    )}
                  ...
                  <div className="layout-switch" aria-label="Diff layout" role="group">
                    <button aria-pressed={layout === 'adaptive'} onClick={() => setLayout('adaptive')} type="button">Adaptive</button>
                    <button aria-pressed={layout === 'unified'} onClick={() => setLayout('unified')} type="button">Unified</button>
                  </div>
                  {settings.keyboardLayout === 'vim' ? (
                    <button
                      ...
                      type="button"
                    >
                      {vimVisualMode ? 'Vim · Visual' : vimDiffMode ? 'Vim · Normal' : 'Enter Vim'}
                    </button>
                  ) : null}
                  <button
                    ...
                    onClick={() => reviewPanelOpen ? closeReviewPanel(false) : openReviewPanel('panel')}
                    ref={reviewToggleRef}
                  ...
                  <button
                    ...
                    disabled={!displayedDiff || displayedDiff.reviewStatus === 'approved' || approving === 'file'}
                    onClick={() => void approveFile()}
                    ref={approveFileButtonRef}
                    type="button"
                  >
                    <CheckIcon />
                    {approving === 'file' ? 'Approving file' : displayedDiff?.reviewStatus === 'approved' ? 'File approved' : 'Approve file'}
                  </button>
                </div>
              </header>

              {settings.keyboardLayout === 'vim' && vimDiffMode ? (
                <div aria-live="polite" className="vim-mode-guide" role="status">
                  <strong>{vimVisualMode ? 'Visual line' : 'Normal'}</strong>
                  <span><kbd>J</kbd><kbd>K</kbd> move</span>
                  <span><kbd>Ctrl+D</kbd><kbd>Ctrl+U</kbd> page</span>
                  <span><kbd>gg</kbd><kbd>G</kbd> ends</span>
                  <span><kbd>V</kbd> {vimVisualMode ? 'finish selection' : 'select'}</span>
              ...
              {reviewHintVisible && displayedDiff ? (
                <div className="review-hint" role="note">
                  ...
                  <button onClick={dismissReviewHint} type="button">Got it</button>
                </div>
              ) : null}
              ...
              {draftInvalidationNotice ? (
                <div className="draft-invalidation-notice" role="status">
                  ...
                  <button onClick={() => setDraftInvalidationNotice('')} type="button">Dismiss</button>
                </div>
              ) : null}
              {diffLoading ? (
                <div className="diff-loading" aria-label="Loading file diff">
                  {Array.from({ length: 14 }, (_, index) => (
                    <span className="skeleton skeleton-code" key={index} style={{ width: `${44 + ((index * 19) % 50)}%` }} />
                  ))}
                </div>
              ) : displayedDiff ? (
                <div className="diff-scroll" data-vim-mode={settings.keyboardLayout === 'vim' && vimDiffMode}>
                  {displayedDiff.truncated ? <p className="truncated-notice">Showing the first 5 MB of this file.</p> : null}
                  <DiffView
                    canExpandContext={contextLines < 20}
                    diff={displayedDiff}
                    onExpandContext={() => setContextLines((current) => current < 8 ? 8 : 20)}
                ...
                <div className="diff-error-empty">
                  ...
                  <span>{actionError || 'Refresh the workspace and choose the file again.'}</span>
                  <button onClick={() => void loadDiff()} type="button">Retry diff</button>
                </div>
              )}
              {selectedLines.length > 0 && !reviewPanelOpen ? (
                <div className="line-selection-toolbar" role="status">
                  ...
                  <span>{settings.keyboardLayout === 'vim' && vimDiffMode
                    ? 'J/K extends in Visual line'
                    : 'Shift extends, Ctrl or Cmd toggles'}</span>
                  <button onClick={() => setSelectedLines([])} type="button">Clear</button>
                  <button
                    aria-keyshortcuts="c"
              ...
              {pendingCommentDeletion ? (
                <div className="comment-delete-undo" role="status">
                  <span>Review note removed from <strong>{pendingCommentDeletion.comment.path}</strong>.</span>
                  <button autoFocus onClick={undoCommentDeletion} type="button">Undo</button>
                </div>
              ) : null}
            </>
          ) : workspace.files.length === 0 ? (
            <div className="clean-workspace-state">
              <span className="clean-mark"><CheckIcon /></span>
              <p className="eyebrow">Working tree</p>
              <h2>No local changes.</h2>
              <p>Redline is watching this checkout. Modified files will appear here without a browser upload.</p>
              <button onClick={() => void loadWorkspace(true)} type="button"><RefreshIcon /> Check again</button>
            </div>
          ) : (
            <div className="clean-workspace-state">
              <span className="clean-mark"><CheckIcon /></span>
              ...
              <p>Choose Approved or All to revisit a file. If its bytes change, it returns to Needs review.</p>
            </div>
          )}
        </section>
        ...
        >
          <div className="ledger-heading">
            <div>
              ...
              <h2>{workspace.latestSnapshot ? 'Current snapshot' : 'No snapshot yet'}</h2>
            </div>
            <span className="ledger-status" data-state={workspace.latestSnapshot?.changedCount ? 'changed' : 'current'}>
              {workspace.latestSnapshot?.changedCount ? `${workspace.latestSnapshot.changedCount} changed` : workspace.latestSnapshot ? 'Holds' : 'Open'}
            </span>
            <button
          ...
          <div className="snapshot-status">
            {workspace.latestSnapshot ? (
              workspace.latestSnapshot.changedCount > 0 ? (
                <>
                  <strong>{workspace.latestSnapshot.changedCount} file{workspace.latestSnapshot.changedCount === 1 ? '' : 's'} changed since approval.</strong>
                  <p>Only those files need another look.</p>
                </>
              ) : (
                <>
                  <strong>Snapshot holds.</strong>
                  <p>{workspace.latestSnapshot.unchangedCount} approved file{workspace.latestSnapshot.unchangedCount === 1 ? '' : 's'} remain unchanged.</p>
                </>
              )
            ) : (
              <>
                <strong>Approve a point in time.</strong>
                <p>Redline will remember the exact file fingerprints you reviewed.</p>
              </>
            )}
          </div>

          <dl className="ledger-facts">
            <div><dt>Needs review</dt><dd>{workspace.counts.needsReview}</dd></div>
            <div><dt>Still approved</dt><dd>{workspace.counts.approved}</dd></div>
            <div><dt>Last approved</dt><dd>{formatRelativeTime(workspace.latestSnapshot?.approvedAt)}</dd></div>
          </dl>
          ...
          >
            <span><CheckIcon /></span>
            <span>
              <strong>{approving === 'snapshot' ? 'Approving snapshot' : 'Approve current snapshot'}</strong>
          ...
          <section className="comments-section" aria-label="Review comments">
            ...
            {selectedLines.length > 0 ? (
              <form className="comment-composer" onSubmit={(event) => void addComment(event)}>
                <label htmlFor="comment-body">{selectionLabel(selectedLines)}</label>
                <textarea
                  ...
                  onKeyDown={(event) => {
                    if (
                      ...
                      commentBody.trim() && !savingComment
                    ) {
                      event.preventDefault();
                  ...
                  value={commentBody}
                />
                <p className="composer-shortcut"><kbd>Ctrl</kbd>/<kbd>⌘</kbd> + <kbd>Enter</kbd> saves</p>
                <div>
                  <button className="text-button" onClick={() => setSelectedLines([])} type="button">Clear selection</button>
                  <button className="save-comment-button" disabled={!commentBody.trim() || savingComment} type="submit">
            ...
            <div className="comment-list">
              {displayedDiff?.comments.map((comment) => (
                <article className="review-comment" data-outdated={comment.outdated} key={comment.id}>
                  <header>
                    <span>{comment.outdated ? `Was ${commentLineLabel(comment)}` : commentLineLabel(comment)}</span>
                    {comment.outdated ? <em>Stale anchor</em> : <time>{formatRelativeTime(comment.createdAt)}</time>}
                  </header>
                  <p>{comment.body}</p>
                  {comment.outdated ? (
                    <p className="stale-comment-note">The file changed after this note. It stays on the reviewed version and is not attached to the current line.</p>
                  ) : null}
                  <button onClick={() => scheduleCommentDeletion(comment)} type="button">Delete note</button>
                </article>
              )) ?? null}
              {displayedDiff && displayedDiff.comments.length === 0 && selectedLines.length === 0 ? (
                <p className="comments-empty">Select line numbers to leave a local note.</p>
              ) : null}
            </div>
          </section>

          <p className="local-promise"><LockIcon /> Files and review state stay on this machine.</p>
        </aside>
      </div>
    </main>
  );
    ...
    <form className="open-workspace-form" onSubmit={onSubmit}>
      ...
      <div>
        <input
          ...
          value={path}
        />
        <button disabled={loading || !path.trim()} type="submit">{loading ? 'Opening workspace' : 'Open workspace'}</button>
      </div>
      {error ? <p id="workspace-path-error" role="alert">{error}</p> : null}
      <span>Only the path is sent to Redline's localhost service. Browser file upload is not used.</span>
    </form>
  );
}
